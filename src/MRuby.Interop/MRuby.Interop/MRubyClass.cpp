...
namespace MRubyCSharp {
	namespace Interop {
		...
		void MRubyClass::DefineClass()
		{
			...
			DFree ^dfree = gcnew DFree(this, &MRubyClass::Free);
			this->gcHandle = (IntPtr)GCHandle::Alloc(dfree);
			IntPtr ^fp = Marshal::GetFunctionPointerForDelegate(dfree);
		...
		MRubyClass::MethodBase::MethodBase(MRubyClass ^klass, String^ name)
		{
			...
			MRubyFunc ^func = gcnew MRubyFunc(this, &MRubyClass::MethodBase::Func);
			this->gcHandle = (IntPtr)GCHandle::Alloc(func);
			IntPtr ^fp = Marshal::GetFunctionPointerForDelegate(func);
